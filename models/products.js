...
module.exports = {
    ...
    async createAProduct(req, res) {
        try {
            const { varenummer, prisenhed, beskrivelse, enhedsbetegnelse, indkøbspris, billede, EAN, vareundergruppe } = req.body;
            ...
            let vare = [varenummer,prisenhed,beskrivelse,enhedsbetegnelse,indkøbspris,billede,EAN,vareundergruppe];
    ...
    async updateAProduct(req, res) {
        try {
            const { id } = req.params;
            const {prisenhed, beskrivelse, enhedsbetegnelse, indkøbspris, billede, EAN, vareundergruppe } = req.body;
            let sql = `update vare SET prisenhed=?, beskrivelse=?, enhedsbetegnelse=?, indkøbspris=?, billede=?, EAN=?, vareundergruppe=? WHERE varenummer = ?`
            let vare = [prisenhed,beskrivelse,enhedsbetegnelse,indkøbspris,billede,EAN,vareundergruppe, id];
            await pool.query(sql, vare);            
            return true;
        } catch (e) {
            console.error(e.message);
